...
where $\mu_j^{\mathrm{Hess}}$ are the three eigenvalue clusters of the Hessian $D^2\Phi$ from Axiom II, and the integration is over the spectrum of the Hessian alone (computed before any operator is constructed).

This functional depends only on the Hessian eigenvalues, not on any Yang-Mills operator eigenvalues or coupling-dependent spectral properties.
...
\item[\textbf{(ii) Functional as Implicit Condition:}] Rather than viewing $\mathcal{F}[\mathbf{w}]$ as a prior definition that determines $\mathbf{w}$, we view it as the \textit{implicit condition} that the weights must satisfy. That is:
\begin{quote}
``Find normalized weights $\mathbf{w} = (w_1, w_2, w_3)$ such that the functional $\mathcal{F}[\mathbf{w}]$ (defined in terms of those same weights and the fixed Hessian data) attains a critical point:''
$$\frac{d\mathcal{F}}{d\alpha}[\mathbf{w}(\alpha_c)] = 0.$$
\end{quote}
