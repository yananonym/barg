...
\begin{proof}
\input{proofOTheoremSeeleyDewitt}
\end{proof}
\end{theorem}
...
\begin{proof}
\input{proofOLemmaIrLimitCommutes}
\end{proof}
\end{lemma}
...
\begin{proof}
\input{proofOTheoremEinsteinHilbertEmergence}
\end{proof}
\end{theorem}
...
\begin{proof}
\input{proofOTheoremScalingLimitGravity}
\end{proof}
...
\begin{proof}
\input{proofOTheoremGravitationalConvolution}
\end{proof}
\end{theorem}
...
\begin{remark}[Extension to Full Higher-Derivative Truncation]
\label{rem:extensiontofullhigherderivativetruncation}

The analysis above covers the minimal two-loop contribution. A complete higher-derivative truncation would include $R^3, R_{\mu\nu}R^{\mu\nu}R^{\rho\sigma}R_{\rho\sigma}$, etc., arising at three-loop and higher. For CMI publication, the two-loop analysis suffices as proof-of-principle; the full tower requires systematic functional RG methods and is left for future work.
